    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="flex justify-between items-center mb-6">
                <h2 class="text-2xl font-bold text-gray-800">Editar Expediente: {{ $cliente->persona->nombre }}</h2>
                ...
                <form action="{{ route('clientes.update', $cliente) }}" method="POST">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <input type="text" name="nombre" value="{{ old('nombre', $cliente->persona->nombre) }}" 
                                   class="mt-1 w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500">
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700">Apellido Paterno</label>
                            <input type="text" name="apellido_paterno" value="{{ old('apellido_paterno', $cliente->persona->apellido_paterno) }}" 
                                   class="mt-1 w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500">
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700">N° Documento (CURP/INE)</label>
                            <input type="text" name="numero_documento" value="{{ old('numero_documento', $cliente->persona->numero_documento) }}" 
                                   class="mt-1 w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500">
                        ...
                        <div class="col-span-1 md:col-span-2">
                            ...
                            <textarea name="direccion" rows="2" class="mt-1 w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500">{{ old('direccion', $cliente->persona->direccion) }}</textarea>
                        </div>
                    </div>
